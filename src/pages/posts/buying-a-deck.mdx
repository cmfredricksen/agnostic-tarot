...
{/* </div> */}
Agnostic Tarot uses the Tarot de Marseille deck. It is a so-called "pip deck", meaning the number cards do not have scenes on them. They are like playing cards, showing just the suit symbol times the number, the 7 of cups for example only shows 7 cups. There are flourishes, flowers and other decorative elements - but there is no scene with a person doing something, unlike most tarot decks today.
